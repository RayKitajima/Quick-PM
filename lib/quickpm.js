...
let api_console = function(query,callback){
	callback(false, { Console: MergedConsole });
};
	...
	if( should_start || force ){
		...
		proc.stdout.on('data',function(data){
			...
			if( Config.Setting.merge ){
				MergedConsole.push( "[" + name + "][stdout] " + data.toString() );
			}
		});
		proc.stderr.on('data',function(data){
			...
			if( Config.Setting.merge ){
				MergedConsole.push( "[" + name + "][stderr] " + data.toString() );
			}
		});
		status = 'up';
	}
